...
CREATE OR REPLACE FUNCTION  public.tr_fc_orderlines_delete()
    RETURNS trigger
    LANGUAGE 'plpgsql'
    COST 100
...
BEGIN
	IF(OLD."AnimalProductId" IS NOT NULL) THEN
		UPDATE "AnimalProducts"  SET "quantityAvailable" = "quantityAvailable" + OLD."quantity"
		 WHERE "id" = OLD."AnimalProductId";
	ELSIF(OLD."EggsBatchProductId" IS NOT NULL) THEN
		UPDATE "EggsBatchProducts"  SET "quantityAvailable" = "quantityAvailable" + OLD."quantity"
		 WHERE "id" = OLD."EggsBatchProductId";
	END IF;
	
	UPDATE "Orders" SET "total" = "total" - OLD."total",
